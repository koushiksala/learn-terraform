...
resource "null_resource" "backend" {
  ...
  git pull
  sleep 60
  ansible-playbook -i ${aws_instance.backend.private_ip}, -e ansible_user=centos -e ansible_password=DevOps321 expense.yml -e role_name=backend
  EOF
  }
}
...
resource "null_resource" "mysql" {
  ...
  git pull
  sleep 60
  ansible-playbook -i ${aws_instance.mysql.private_ip}, -e ansible_user=centos -e ansible_password=DevOps321 expense.yml -e role_name=mysql
  EOF
  }
}